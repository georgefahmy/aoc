...
commands = [
    (n.split()[0], n.strip().split()[1]) if "add" in n else (n.strip(), None)
    for n in open("2022/data/10.txt", "r").readlines()
]
...
# Part 1 Answer
print(
    f" Part 1 Answer: {sum([elf_cpu.cycle_signal_history.get(v) for v in list(range(20, 221, 40))])}"
)
...
for panel in panels:
    for line in [panel[i : i + 40] for i in range(0, 240, 40)]:
        sprites = [
            n % 40 in list(map(lambda x: x + elf_cpu.cycle_register_history.get(n), range(0, 3, 1)))
            for n in line
        ]
